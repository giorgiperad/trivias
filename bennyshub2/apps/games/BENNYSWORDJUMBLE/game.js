...
class WordJumbleGame {
    constructor() {
        ...
        this.settings = {
            ...
            dataSource: 'server' // 'server', 'local', 'custom'
        };
    ...
    loadWordsSource() {
        if (this.settings.dataSource === 'local') {
            const local = localStorage.getItem('wordjumble_custom_words');
            if (local) {
                try {
                    this.wordsData = JSON.parse(local);
                    console.log("Loaded Local Storage Words");
                } catch(e) {
                    console.error("Corrupt local data, reverting to server");
                    this.wordsData = this.serverWords;
                    this.settings.dataSource = 'server';
                }
            } else {
                this.wordsData = this.serverWords; // Fallback
            }
        } else {
            this.wordsData = this.serverWords;
        }

        // Recalculate available lengths/difficulties
    ...
    toggleDataSource() {
        // Toggle between Server and Local (if available)
        if (this.settings.dataSource === 'server') {
            if (localStorage.getItem('wordjumble_custom_words')) {
                this.settings.dataSource = 'local';
                this.speak("Using My Custom Words");
            } else {
                this.speak("No custom words saved.");
                return; // Don't switch if no data
            }
        } else {
            this.settings.dataSource = 'server';
            this.speak("Using Default Words");
        }
        this.saveSettings();
        this.loadWordsSource();
                ...
                reader.onload = (ev) => {
                    try {
                        ...
                        if (Array.isArray(data)) {
                            this.wordsData = data;
                            this.settings.dataSource = 'custom'; // Temp session source
                            
                            // Recalculate
                            const lengths = new Set(this.wordsData.map(w => w.word.length));
                            difficulties.casual = Array.from(lengths).filter(l => l >= 2).sort((a,b) => a - b);
                            
                            this.loadHighScores(); // Update high score for custom file
                            this.saveSettings();
                            this.renderSettingsMenu();
                            this.speak("Loaded Custom File");
                        }
                    } catch(err) {
                        alert("Invalid JSON");
                    }
    ...
    renderSettingsMenu() {
        ...
        container.innerHTML = `
            ...
            <div id="menu-list" style="display: grid; grid-template-columns: 1fr 1fr; gap: 2vmin; width: 80%; max-width: 1000px; margin: 0 auto; box-sizing: border-box;">
                ...
                <button class="menu-button" style="width: 100%; margin: 0; font-size: 3vmin; white-space: normal; height: 100%; min-height: 8vh;" onclick="game.toggleDataSource()">Source: ${s.dataSource === 'server' ? 'Default' : (s.dataSource === 'local' ? 'My Saved' : 'Custom')}</button>
                ...
                <button class="menu-button" style="width: 100%; margin: 0; font-size: 3vmin; white-space: normal; height: 100%; min-height: 8vh; grid-column: span 2;" onclick="game.backFromSettings()">Back</button>
            </div>
        `;
    ...
    startChallengeGame(hp) {
        this.maxHP = hp;
        this.currentHP = hp;
        this.currentLevelLength = 2; // Start with 2 letter words
        this.levelProgress = 0;
        this.usedWords.clear();
    ...
    nextLevel() {
        ...
        if (candidates.length === 0) {
            if (this.gameMode === 'challenge') {
                ...
                const nextCheck = this.wordsData.filter(w => w.word.length === this.currentLevelLength);
