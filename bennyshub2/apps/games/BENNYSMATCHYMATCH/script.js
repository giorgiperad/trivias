    ...
    try {
        ...
        localReg.forEach(p => {
             ...
             if (existingIndex !== -1) {
                 ...
                 if (state.packs[existingIndex] !== p) {
    ...
    try {
        ...
        if (localContent) {
            ...
            if (data && data.categories) {
                state.assets.categories = data.categories;
                console.log("Pack loaded from Browser Storage:", filename);
                return;
            }
        }
    } catch(e) {
...
const menus = {
    main: [
        { 
            text: () => `Match Category: ${getPackTitle()}`, 
            action: () => cyclePack(1),
            onPrev: () => cyclePack(-1)
        },
    ...
    loadWarning: [
        { text: "Continue (Mouse Needed)", action: () => promptLoadGame() },
        ...
        reader.onload = event => {
            try {
                ...
                if (data && data.categories) {
                    ...
                    speak("Invalid Game File");
                    alert("Invalid Game File: Missing categories");
                }
            } catch (err) {
                console.error(err);
                speak("Error loading file");
...
function startGame(difficulty) {
    ...
    state.scan = { row: 0, col: 0, mode: 'row' };
    state.mismatches = 0;
    ...
    state.grid.forEach(cell => {
        ...
        if (!cell.inactive) {
            ...
            if (cell.matched) {
                ...
                // Smart path handling
                if (cell.image.startsWith('http') || cell.image.startsWith('data:')) {
                    img.src = cell.image;
                } else if (cell.image.startsWith('assets/')) {
                    img.src = cell.image;
                } else {
                    img.src = `assets/${cell.image}`;
                }
...
function moveGameScan(dir) {
    state.ignoreHover = true;
    if (state.scan.mode === 'row') {
        let nextRow = state.scan.row;
        for (let i = 0; i < state.rows; i++) {
            nextRow = (nextRow + dir + state.rows) % state.rows;
            if (rowHasUnmatched(nextRow)) {
...
function playSound(name) {
    ...
    if (card.sound && settings.sound) {
        ...
        if (soundFile) {
            let src = soundFile;
            if (!soundFile.startsWith('http') && !soundFile.startsWith('data:') && !soundFile.startsWith('assets/')) {
                src = `assets/${soundFile}`;
            }
