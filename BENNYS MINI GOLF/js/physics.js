class PhysicsEngine {
    ...
    update(balls, walls, waters, sands, ice, boosts, bridges, trees, dt) {
        ...
        ballArray.forEach(ball => {
            ...
            if (inIce) currentFriction = 0.998; // Very low friction (Ice)
            ...
            if (boosts) {
                boosts.forEach(boost => {
                    let inBoost = false;
                    if (boost.points) {
                        if (Utils.pointInPolygon(nextX, nextY, boost.points)) inBoost = true;
                    }
