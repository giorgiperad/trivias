class Game {
    ...
    update(dt) {
        ...
        if (this.canShoot) {
            if (this.aimRotating) {
                ...
                if (speedSetting === 'Slow') speed = 12; // 360/30
    ...
    getPredictedPath() {
        ...
        for (let i = 0; i < maxSteps; i++) {
            this.physics.update(ghostBall, this.walls, this.waters, this.sands, this.ice, this.boosts, this.bridges, this.trees, dt);
    ...
    draw() {
        ...
        if (this.state === 'PLAYING' || this.state === 'HOLE_OUTRO' || this.state === 'HOLE_INTRO') {
            ...
            this.boosts.forEach(b => {
                this.ctx.fillStyle = this.boostPattern || '#FFA500'; 
