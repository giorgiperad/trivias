class MenuSystem {
    ...
    async showLevelSelect() {
        ...
        try {
            ...
            console.warn("Could not load courses from server, using fallback.", e);
            courses = [
                { name: "Benny's Backyard", file: "courses/bennys_backyard.json" }
            ];
        }

        this.items = courses.map(c => ({
            text: c.name,
            action: () => this.startGame(c.file)
        }));
        ...
        this.items.push({ text: 'Back', action: () => this.showMainMenu() });
    ...
    exitGame() {
        ...
        window.close(); // May not work in all browsers/contexts
        // If running in a wrapper or specific environment, this might need adjustment
